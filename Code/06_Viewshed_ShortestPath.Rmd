...
library(rgrass7)
loc <-  initGRASS(gisBase = "/opt/grass/", home=tempdir() ,mapset = "PERMANENT", override = TRUE)
...
cum_view_sites <- brick(raster(srtm))
...
for (i in seq(1, length(sites@coords[,1]))) {
    ...
    viewshed <- readRAST("view")
    cum_view_sites[[i]] <- raster(viewshed)
    names(cum_view_sites[[i]]) <- as.character(sites@data$Nr.[i])
...
```{r}
view_sum_sites <- sum(cum_view_sites)
plot(view_sum_sites)
```
...
```{r}
quantile(view_sum_sites)

rcl_sites <- c(-Inf, quantile(view_sum_sites)[2],1,
               quantile(view_sum_sites)[2],quantile(view_sum_sites)[3],2,
               quantile(view_sum_sites)[3],quantile(view_sum_sites)[4],3,
               quantile(view_sum_sites)[4],quantile(view_sum_sites)[5],4,
               quantile(view_sum_sites)[5],+Inf,5
               )
rcl_sites <- matrix(rcl_sites, ncol = 3, byrow = TRUE)
view_sum_sites2 <- reclassify(x = view_sum_sites, rcl = rcl_sites)
```
...
```{r}
plot(view_sum_sites2)
points(sites, pch=19, cex=.5)

#library(rasterVis)
#levelplot(view_sum_sites2)
```
