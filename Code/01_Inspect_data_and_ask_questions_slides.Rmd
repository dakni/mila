...
author: Freie Universität Berlin -- M.Sc. Landschaftsarchäologie <br> Modellieren in der Landschaftsarchäologie <br> Dr. Daniel Knitter <br>
date: WS 2015/2016 <br> 2015-12-09
...
```{r}
a <- data.frame(A = c(TRUE,TRUE,FALSE),
...
## a small read and a bit of R

1. Please read chapters 2 and 4 of:
    - VanPool, Todd L., and Robert D. Leonard. 2011. Quantitative Analysis in Archaeology. Chichester, West Sussex, U.K. ; Malden, MA: Wiley-Blackwell.
    - available at: https://userpage.fu-berlin.de/~dare211/VS_Modellierung/VanPool_Leonard_2011_ch_02_04.pdf
2. Calculate - for the dataset that is introduced today - the different measures of central tendency and dispersion.
...
```{r}
head(names(raw.data),3)
head(sapply(raw.data, class),3)
```
...
```{r}
head(levels(raw.data$Epoch..site.),3)
tail(levels(raw.data$Site.type.), 3)
```
...
classic
[1. select relevant data, 2. check whether everything is fine, 3. drop unnecessary columns]
