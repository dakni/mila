...
```{r trendsurf, fig.show="hide"}
lecp2 <- lm(lec@data$rad ~ poly(x = lec@coords[,1],
                                y = lec@coords[,2],
                                degree = 2
                                )
            )
lecp3 <- lm(lec@data$rad ~ poly(x = lec@coords[,1],
                                y = lec@coords[,2],
                                degree = 3
                                )
            )
...
lecp4 <- lm(lec@data$rad ~ poly(x = lec@coords[,1],
                                y = lec@coords[,2],
                                degree = 4
                                )
          )
...
lines(lecp2$fitted.values, col = "red")
lines(lecp3$fitted.values, col = "blue")
lines(lecp4$fitted.values, col = "green")
legend("top",
       lty = c(1,1,1),
       col = c("red","blue","green"),
       legend = c("2nd degree polynom",
...
**very** superficial and I hope you can do better 
...
plot(variogram(lec$rad ~ 1, lec, cutoff = 60000))
```
...
plot(variogram(lec$rad ~ 1, lec, cutoff = 60000, alpha = c(0,45,90,135)))
```
...
plot(variogram(object = lec$rad ~ 1,
               locations = lec,
               width = 1000,
               cutoff = 50000)
     )
